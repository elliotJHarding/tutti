"""tutti doctor — validate the full prerequisite chain."""
...
from tutti.cli.output import error, get_json_mode, output, section, success, warn
...
def doctor(ctx: click.Context) -> None:
    ...
    try:
        ...
        error("Cannot continue without a workspace. Run 'tutti init' first.")
        ctx.exit(1)
        return
    ...
    if "zsh" in shell:
        ...
        activation = 'autoload -Uz compinit && compinit -C 2>/dev/null; eval "$(_TUTTI_COMPLETE=zsh_source tutti)"'
    elif "bash" in shell:
        shell_name, rc_path = "bash", Path.home() / ".bashrc"
        activation = 'eval "$(_TUTTI_COMPLETE=bash_source tutti)"'
    elif "fish" in shell:
        shell_name, rc_path = "fish", Path.home() / ".config" / "fish" / "config.fish"
        activation = '_TUTTI_COMPLETE=fish_source tutti | source'
    else:
        shell_name, rc_path, activation = None, None, None

    if shell_name and rc_path:
        rc_content = rc_path.read_text() if rc_path.exists() else ""
        has_completion = "_TUTTI_COMPLETE" in rc_content
    ...
    if all_ok:
        ...
        warn("Some checks failed. Fix the issues above and re-run 'tutti doctor'.")
        ctx.exit(1)
