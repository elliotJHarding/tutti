"""Tests for the tutti session command."""
...
from tutti.cli.session_cmd import (
...
def test_discover_sessions_pid_files(tmp_path: Path):
    ...
    with patch("tutti.cli.session_cmd._is_pid_alive", return_value=True):
...
def test_discover_sessions_dead_pid(tmp_path: Path):
    ...
    with patch("tutti.cli.session_cmd._is_pid_alive", return_value=False):
        sessions = _discover_sessions(claude_dir=tmp_path)
...
def test_session_list_no_sessions(tmp_path: Path):
    ...
    with patch("tutti.cli.session_cmd._discover_sessions", return_value=[]):
...
def test_session_list_json(tmp_path: Path):
    ...
    with patch("tutti.cli.session_cmd._discover_sessions", return_value=_mock_sessions()):
...
def test_session_list_json_grouped_by_ticket(tmp_path: Path):
    ...
    with patch("tutti.cli.session_cmd._discover_sessions", return_value=sessions):
...
def test_session_jump_activates_tab(tmp_path: Path):
    ...
    with (
        patch("tutti.cli.session_cmd._discover_sessions", return_value=sessions),
        patch("tutti.cli.session_cmd._get_tty", return_value="ttys026"),
        patch("tutti.cli.session_cmd._focus_terminal_tab", return_value=True),
    ):
        result = runner.invoke(
...
def test_session_jump_no_tty(tmp_path: Path):
    ...
    with (
        patch("tutti.cli.session_cmd._discover_sessions", return_value=sessions),
        patch("tutti.cli.session_cmd._get_tty", return_value=None),
    ):
        result = runner.invoke(
            cli, ["--workspace-root", str(tmp_path), "session", "jump", "active"]
        )
...
def test_session_jump_no_terminal(tmp_path: Path):
    ...
    with (
        patch("tutti.cli.session_cmd._discover_sessions", return_value=sessions),
        patch("tutti.cli.session_cmd._get_tty", return_value="ttys026"),
        patch("tutti.cli.session_cmd._focus_terminal_tab", return_value=False),
    ):
        result = runner.invoke(
            cli, ["--workspace-root", str(tmp_path), "session", "jump", "active"]
        )
