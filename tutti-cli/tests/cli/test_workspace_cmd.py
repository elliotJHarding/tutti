...
from tutti.cli.workspace_cmd import discover_repos, find_repo, list_branches
from tutti.config import WorkspaceConfig
...
def test_add_repo_top_level_all_args(tmp_path: Path) -> None:
    """tutti add-repo KEY REPO BASEBRANCH should create a worktree (top-level command)."""
    runner = CliRunner()
    _init_workspace(runner, tmp_path)
...
def test_add_repo_workspace_alias(tmp_path: Path) -> None:
    """tutti workspace add-repo should still work as an alias."""
    runner = CliRunner()
    _init_workspace(runner, tmp_path)
